...
const customerItems = computed(() => [
    ...
    { href: '/collections', label: 'Collections', icon: Layers, show: permissions.value['stores.view'] || permissions.value['stores.manage'] || permissions.value['stores.sync'], locked: !planAccess.value.collections },
    { href: '/rank-tracking', label: 'Keyword Tracking', icon: LineChart, show: true, locked: !planAccess.value.rank_tracking },
    { href: '/ai-visibility', label: 'AI Visibility', icon: Bot, show: true, locked: !planAccess.value.ai_visibility },
    { href: '/topics', label: 'Topics', icon: Lightbulb, show: permissions.value['topics.manage'], locked: !planAccess.value.topics },
    { href: '/blogs', label: 'Blogs', icon: FileText, show: permissions.value['blogs.edit'] || permissions.value['blogs.approve'], locked: !planAccess.value.blogs },
    { href: '/blogs?status=scheduled', label: 'Schedule', icon: CalendarClock, show: permissions.value['blogs.approve'], locked: !planAccess.value.schedule },
]);
        ...
        <aside class="fixed inset-y-0 left-0 hidden w-64 border-r border-zinc-200 bg-white lg:block">
            ...
            <nav class="space-y-1 p-3">
                <Link
                    ...
                    :class="isActive(item.href) ? 'bg-teal-50 text-teal-800' : (item.locked ? 'text-zinc-500 hover:bg-zinc-100' : 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950')"
                >
